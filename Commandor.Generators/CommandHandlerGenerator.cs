...
public class CommandHandlerGenerator : IIncrementalGenerator
{
    ...
    private static string GenerateHandlers(INamedTypeSymbol interfaceSymbol, List<IMethodSymbol> handlerMethods, Compilation compilation)
    {
        ...
        {
            var handlerSource = GenerateHandler(interfaceSymbol, method, compilation, proxyMethods);
            if (!string.IsNullOrEmpty(handlerSource))
                sb.AppendLine(handlerSource);
        }

        // Emit a proxy struct exposing query methods, plus a C# 14 extension property
        // on ICommandor so callers write:  commandor.<ServicePrefix>.<Method>(...)
        if (proxyMethods.Count > 0)
        {
            var servicePrefix = GetServicePrefix(interfaceSymbol);
            var proxyTypeName = $"{servicePrefix}CommandorProxy";

            sb.AppendLine($"    /// <summary>Auto-generated proxy exposing <c>{interfaceSymbol.Name}</c> query methods on <c>ICommandor</c>.</summary>");
            sb.AppendLine($"    public readonly struct {proxyTypeName}");
            sb.AppendLine("    {");
            sb.AppendLine("        private readonly global::Commandor.ICommandor _commandor;");
            sb.AppendLine($"        public {proxyTypeName}(global::Commandor.ICommandor commandor)");
            sb.AppendLine("            => _commandor = commandor ?? throw new global::System.ArgumentNullException(nameof(commandor));");
            sb.AppendLine();
            foreach (var pm in proxyMethods)
                sb.Append(pm);
            sb.AppendLine("    }");
            sb.AppendLine();
            sb.AppendLine($"    /// <summary>Adds <c>{servicePrefix}</c> extension property to <c>ICommandor</c> for accessing <c>{interfaceSymbol.Name}</c> queries.</summary>");
            sb.AppendLine($"    public static class Commandor{servicePrefix}Extensions");
            sb.AppendLine("    {");
            sb.AppendLine("        extension(global::Commandor.ICommandor commandor)");
            sb.AppendLine("        {");
            sb.AppendLine($"            /// <summary>Access <c>{interfaceSymbol.Name}</c> query methods.</summary>");
            sb.AppendLine($"            public {proxyTypeName} {servicePrefix} => new(commandor);");
            sb.AppendLine("        }");
            sb.AppendLine("    }");
            sb.AppendLine();
        }
        ...
        List<string> proxyMethods)
    {
        ...
        if (plainParams.Count == 0)
        {
            ...
            extNoArg.AppendLine($"        public global::System.Threading.Tasks.Task<{responseTypeName}> {method.Name}(");
            extNoArg.AppendLine("            global::System.Threading.CancellationToken cancellationToken = default)");
            extNoArg.AppendLine($"            => _commandor.GetAsync(new {noArgWrapperName}(), cancellationToken);");
            extNoArg.AppendLine();
            proxyMethods.Add(extNoArg.ToString());
        ...
        if (isIRequestMode)
        {
            ...
            // Proxy method for QueryHandlers: commandor.<Service>.<Method>(request) → _commandor.GetAsync(request)
            if (isQueryHandler && responseType != null && responseTypeName != null)
            {
                ...
                ext.AppendLine($"        public global::System.Threading.Tasks.Task<{responseTypeName}> {method.Name}(");
                ext.AppendLine($"            {requestTypeName} request,");
                ext.AppendLine($"            global::System.Threading.CancellationToken cancellationToken = default)");
                ext.AppendLine($"            => _commandor.GetAsync(request, cancellationToken);");
                ext.AppendLine();
                proxyMethods.Add(ext.ToString());
            }
        ...
        // Proxy method: exposes raw params so callers never touch the internal wrapper record.
        ...
        extp.AppendLine($"        public global::System.Threading.Tasks.Task<{responseTypeName}> {method.Name}(");
        extp.AppendLine($"            {extParamList},");
        extp.AppendLine($"            global::System.Threading.CancellationToken cancellationToken = default)");
        extp.AppendLine($"            => _commandor.GetAsync(new {wrapperName}({extCtorArgs}), cancellationToken);");
        extp.AppendLine();
        proxyMethods.Add(extp.ToString());
