...
/// <summary>
/// Compute context - tracks current computation for dependency tracking.
/// Similar to Fusion's Computed.GetCurrent().
/// </summary>
public static class ComputeContext
{
    private static readonly AsyncLocal<IComputed?> _current = new();
    ...
    /// <summary>
    /// Execute action with a compute context.
    /// </summary>
    public static T WithContext<T>(IComputed computed, Func<T> action)
    {
        var prev = Current;
        try
        {
    ...
    /// <summary>
    /// Execute async action with a compute context.
    /// </summary>
    public static async Task<T> WithContextAsync<T>(IComputed computed, Func<Task<T>> action)
    {
        var prev = Current;
        try
        {
